...
class FortunesControllerTest {

	@Mock FortunesService mockService;
	
	private FortunesController subject;
	
	@BeforeEach
	void setup() {
		subject = new FortunesController(mockService);
	}
	...
	@Test
	void controllerReturnsListOfFortunes() {
	    ...
	    when(mockService.fortunes()).thenReturn(expected);
	...
	void serviceReturnsKeyValuePairWhenAKeyIsRecieved() {
	    ...
	    when(mockService.getFortune(14)).thenReturn(expected);
